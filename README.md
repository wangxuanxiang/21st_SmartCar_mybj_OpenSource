...
我们的整个推动规划基于每次推动最多有一个折点，到达折点后直接向目标边垂直方向直接推向目标边。在我们赛题中一个折点几乎能满足大部分情况的最优解最优解，并且也为后续双车协调与状态机编写提供了简便。而我们的BoundaryPathPlanner类也就是为了找到这个最优折点

推动阶段与普通惯导不同，因为主车与从车需要在两侧夹持并推动物体，系统不只要考虑主车自身，还要考虑从车在另一侧的空间占用。因此推动避障采用了特殊膨胀策略。
...
本项目采用主从式双车协同机制：主车负责视觉扫描、目标选择、障碍物处理和整体路径规划，并通过无线串口向从车发送带有目标类型、目标角度和目标坐标的路径指令；从车接收并解析指令后，独立完成解析后导航、视觉伺服和环绕定位。两车通过 R、G、F、L 等状态信号进行阶段握手，分别表示准备完成、伺服完成、环绕完成和目标丢失。主车确认从车完成对应阶段后，再发送下一阶段指令，例如发送 M 指令启动协同推动，使两车按照各自路径同时推动目标物体；任务完成后，从车反馈完成状态，主车进入下一轮目标规划。通过“主车规划、从车执行、状态反馈、阶段确认”的方式，实现了双车在导航、伺服、环绕和推动过程中的协调运行。下一次的路径规划主要依据：上一次推完的last_side,last_posistion（面向推动方向相对从车的左/右）,主车位置，以及下一个物体的推动目标边界

通过状态机控制与无线通讯，可实现以下不同状态流程：

1.主车会根据目标物体物体在主车左/右，判断主/从车哪个先进行视觉伺服，最终逻辑可使两车在运行避障过程中不会相撞
...
由于推动时主车和从车呈 V 型分列在物体两侧，推杆与物体之间的反作用力会使两车逐渐向外张开，造成“外八”、间距变大甚至遗漏物体。为此，从车左右两侧各安装一颗 VL53L4CD TOF 测距传感器，根据当前推动方向选择内侧传感器，实时测量从车与物体（或主车）的距离。
以下是我们对推动所做的一些优化：

1.推动时主车/从车仍然按惯性导航方式计算并输出朝向目标方向的速度作为前馈，tof计算的距离只做x或y轴上单方向的速度改变与闭环控制距离

2.开始时tof控制距离若过小，在斜推时会从旁边漏出，故控制开始（斜推阶段时）tof控距较大，一般比直推大1-2cm,这样在斜推时物体更容易被兜住，大大增加了斜推成功率

3.对沙袋的特殊处理：由于沙袋的阻力较大，在斜推时，内侧车与外侧车收阻力大小不均，内侧容易因为阻力较大赶不上外侧车的速度，导致tof无法扫到，故我们在斜推第一阶段（转折点前）时会对内侧车进行一定加速，这样内侧车就不会落后

4.对网球的特殊处理：由于网球在斜推时，由于斜推时的网球有相对推动方向向左右的移动速度，容易因为惯性直接滚出双车V型区域，我们在网球斜推第二阶段（转折点后）对两车进行加速，通过加速来兜住网球，大大增加了网球斜推成功率

TOF 控制由 `TofControl` 统一管理：主循环中的 `service()` 负责安全地执行 I2C 读写、启动/停止测距和缓存最新数据，控制周期中只读取缓存，避免在定时器回调中进行阻塞式 I2C 操作。测距数据经过有效范围检查、连续无效滞回和滑动滤波后，送入左右独立的距离 PD 控制器 `DistPID`；控制器按网球、沙袋、玩具熊等物体类型选择目标间距，并将修正量叠加到从车横向速度，使两车保持稳定夹持距离。传感器异常或数据超限时立即清零修正量并蜂鸣告警，任务结束后复位测距状态，保证下一次搬运可以重新初始化。
源码定位：[`TofControl`](slave_car/ant_else.py#L487) · [`TofControl.service`](slave_car/ant_else.py#L583) · [`DistPID`](slave_car/ant_motor.py#L708) · [`VL53L4CD`](slave_car/vl53l4cd.py#L66)

### 3.4.8 firstscan 流程与物体九宫格分配

任务开始后，主车先进入 `first_scan` 流程，沿预先生成的扫描路径依次到达扫描点。到达每个扫描点后，系统等待视觉端稳定输出，再连续采集多帧目标列表；每一帧通过坐标变换转换到场地世界坐标，并与上一帧结果进行匹配融合，降低视觉抖动、漏检和偶发误检的影响。连续多帧未收到有效目标时退出当前扫描点，避免因视觉端暂时丢帧而长时间阻塞。
完成全部扫描点后，系统先按物体种类和空间距离合并重复检测结果，再将坐标吸附到场地中央的 3×3 九宫格。每个九宫格最多保留一个目标，并依据目标优先级、数量上限和目标到格心的距离处理冲突；最终生成稳定的九宫格占用表，供 `objects_planner` 判断推动方向、寻找换边空位和计算搬运顺序。第一次扫描结果会被缓存，后续搬运轮次直接使用更新后的剩余物体列表，不再重复进行全场扫描，从而缩短总任务时间。

源码定位：[`first_scan`](main_car/ant_task.py#L515) · [`merge_nearby_same_kind`](main_car/ant_task.py#L424) · [`snap_objects_to_nine_grid`](main_car/ant_task.py#L339) · [`generate_nine_grid`](main_car/ant_boundary_plan.py#L437) · [`objects_planner`](main_car/ant_boundary_plan.py#L364)
